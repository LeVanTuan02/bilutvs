    ...
    <header class="header__content-wrap">
        <div class="header__content">
            <div class="header__content-item">
                ...
                <span class="content__user-text"><?=$totalEpisode?> Total</span>
            </div>

            <div class="header__content-item">
                <a href="<?=DOMAIN?>/?module=backend&controller=film" class="header__content-item-btn">DS Phim</a>
                <a href="<?=DOMAIN?>/?module=backend&controller=episode&action=add&id=<?=$idFilm;?>" class="header__content-item-btn">Thêm tập</a>
            </div>
        </div>
    </header>
            ...
            <table class="product__list-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>Tập <?=$item['tap'];?></td>
                        <td><?=substr($item['link_player'], 0, 30) . '...' . substr($item['link_player'], -5);?></td>
                        <!-- <td>
                        ...
                        <td>
                            <div class="user_list-action">
                                <a onclick="return confirm('Bạn có chắc muốn xóa tập này không?') ? true : false;" href="<?=DOMAIN?>/?module=backend&controller=episode&action=delete&id=<?=$item['id'];?>&id_episode=<?=$item[0];?>" class="user__list-action danger">
                                    <i class="far fa-trash-alt"></i>
                                </a>
                                <a href="<?=DOMAIN?>/?module=backend&controller=episode&action=edit&id=<?=$item['id'];?>&id_episode=<?=$item[0];?>" class="user__list-action info">
            ...
            <ul class="user__list-pagination">
                <?php if ($currentPage > 1 && $totalPage > 1): ?>
                    <li class="user__list-pagination-item">
                        <a href="<?=DOMAIN?>/?module=backend&controller=episode&id=<?=$idFilm;?>&page=1" class="user__list-pagination-link user__list-pagination-link-first">
                            <i class="fas fa-angle-double-left"></i>
                        </a>
                    </li>
                    <li class="user__list-pagination-item">
                        <a href="<?=DOMAIN?>/?module=backend&controller=episode&id=<?=$idFilm;?>&page=<?=($currentPage - 1);?>" class="user__list-pagination-link user__list-pagination-link-pre">
                ...
                <?php
                    for ($i = 1; $i <= $totalPage; $i++) {
                        if ($currentPage == $i) {
                            ...
                            '
                                <li class="user__list-pagination-item">
                                    <a href="'.DOMAIN.'/?module=backend&controller=episode&id='.$idFilm.'&page='.$i.'" class="user__list-pagination-link user__list-pagination-link--active">'.$i.'</a>
                                </li>
                            ';
                        } else {
                            echo
                            '
                                <li class="user__list-pagination-item">
                                    <a href="'.DOMAIN.'/?module=backend&controller=episode&id='.$idFilm.'&page='.$i.'" class="user__list-pagination-link">'.$i.'</a>
                                </li>
                            ';
                        }
                    }
                ...
                <li class="user__list-pagination-item">
                    <a href="<?=DOMAIN?>/?module=backend&controller=episode&id=<?=$idFilm;?>&page=<?=($currentPage + 1);?>" class="user__list-pagination-link user__list-pagination-link-next">
                        <i class="fas fa-angle-right"></i>
                    </a>
                </li>
                <li class="user__list-pagination-item">
                    <a href="<?=DOMAIN?>/?module=backend&controller=episode&id=<?=$idFilm;?>&page=<?=$totalPage;?>" class="user__list-pagination-link user__list-pagination-link-last">
